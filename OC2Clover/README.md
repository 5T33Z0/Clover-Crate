...
|SSDT|Replacement DSDT Fixes
|:--:|----------------------|
`SSDT-AC.aml`| Use `FixAdp1` instead
`SSDT-DTGP` | Doesn't have to be part of SSDT patches. It will be generated by activating `AddDTGP`
...
`SSDT-SBUS-MCHC.aml`|Use Fixes`AddMCHC`and`FixSBUS`instead
