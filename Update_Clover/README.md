...
**`FSInject.efi`** | For Kext injection. Only necessary for legacy versions of macOS ≤ 10.7 (Lion) which are capable of loading individual kexts instead of Prelinkedkernel. Since r5125, `OpenRuntime.efi` handles Kext injection, so FSInject and the corresponding "Inject Kexts" option have become obsolete|Delete
